...
- 流量暴增告警：单 token 1 小时内新增 >3GB 时邮件告警管理员（24h 幂等；不通知客户——暴增多是滥用/泄露，惊动对方只会换号重来），止血用 rotate-uuid 或撤销；给客户续命用 reset-penalty（管理端售后与用户自助同一逻辑：用量清零恢复满额，有效期 -30 天，offset 记账不受 Xray 累计值影响）
