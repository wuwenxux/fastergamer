    ...
    return (
      <div className="max-w-xl mx-auto">
        <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6 space-y-4 text-center">
          <p className="text-xl sm:text-lg font-semibold">此页面需要先登录</p>
          <p className="text-[15px] sm:text-sm text-slate-400">
            请先在「我的 Token」页输入购买邮箱，通过邮件登录链接进入后再登记。
          </p>
          <Link
            to="/tokens"
            className="inline-block rounded-lg bg-sky-500 px-6 py-3 sm:py-2.5 font-medium hover:bg-sky-400 transition-colors"
          >
            前往登录 →
          </Link>
        </div>
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-[15px] leading-relaxed sm:text-sm text-slate-400 mt-2">
      ...
      {!loaded ? (
        <p className="text-[15px] sm:text-sm text-slate-500">加载中…</p>
      ) : (
        <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6 space-y-5">
          <div className="space-y-2">
            <label className="text-[15px] sm:text-sm text-slate-300">通知邮箱（可选，建议备用邮箱）</label>
            <input
              value={notifyEmail}
          ...
          <div className="space-y-2">
            <label className="text-[15px] sm:text-sm text-slate-300">Telegram（可选，@用户名 或 t.me 链接）</label>
            <input
              value={telegram}
          ...
          {saved && !error && (
            <p className="text-emerald-400 text-[15px] sm:text-sm">✅ 已登记。入口有变动时会按此联系方式通知你。</p>
          )}

          <button
            onClick={submit}
            disabled={loading}
            className="w-full rounded-lg bg-sky-500 py-3 sm:py-2.5 font-medium hover:bg-sky-400 transition-colors disabled:opacity-60"
          >
            {loading ? "保存中…" : "保存登记"}
          </button>
