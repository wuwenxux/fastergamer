...
export default function Tokens() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {linkSent && (
        <p className={`text-[15px] sm:text-sm ${linkThrottled ? "text-amber-400" : "text-emerald-400"}`}>
          {linkThrottled
            ...
            {savedTokens.map((t) => (
              ...
              >
                <button
                  ...
                  className="text-left text-[15px] sm:text-sm hover:text-sky-400"
                >
      ...
      {loadingSaved && savedTokens.length === 0 && (
        <p className="text-[15px] sm:text-sm text-slate-500">正在加载历史 Token…</p>
      )}

      {loggedIn && (
        <div className="rounded-xl border border-slate-800 px-4 py-3 flex items-center justify-between">
          <div className="text-[15px] sm:text-sm">
            <span className="text-slate-400">备用联系方式（选填）</span>
            <span className="text-sm sm:text-xs text-slate-500 ml-2">网站换入口或邮箱收不到信时通知你</span>
          </div>
          <Link to="/register" className="text-[15px] sm:text-sm text-slate-400 hover:text-sky-400 shrink-0">
            登记 →
          </Link>
        </div>
      )}
    ...
    <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <div className="font-mono text-[15px] sm:text-sm">{token.id}</div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[15px] sm:text-sm">
        <div className="rounded-lg bg-slate-800/60 p-3">
          <div className="text-slate-400 text-sm sm:text-xs mb-1">套餐</div>
          <div>{token.plan_id}</div>
        </div>
        <div className="rounded-lg bg-slate-800/60 p-3">
          <div className="text-slate-400 text-sm sm:text-xs mb-1">有效期</div>
          <div>
            {token.expires_at
      ...
      <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 space-y-3">
        <p className="text-[15px] sm:text-sm text-amber-300">
...
function LoginLinkInline() {
  ...
  if (state === "sent" || state === "throttled") {
    return (
      <p className={`text-sm leading-relaxed sm:text-xs ${state === "throttled" ? "text-amber-200" : "text-emerald-300"}`}>
        {state === "throttled"
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm leading-relaxed sm:text-xs text-amber-200/70">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-base sm:text-sm outline-none focus:border-sky-500"
        />
        <button
          onClick={send}
          disabled={state === "sending" || !email.includes("@") || (ts.enabled && !ts.token)}
          className="rounded-lg bg-sky-500 px-4 text-base sm:text-sm font-medium hover:bg-sky-400 transition-colors disabled:opacity-60"
        >
          {state === "sending" ? "发送中…" : "发送登录链接"}
        </button>
      </div>
      {err && <p className="text-sm sm:text-xs text-rose-400">{err}</p>}
    </div>
  );
}
